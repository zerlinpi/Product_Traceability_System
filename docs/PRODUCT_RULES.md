...
> **✅ 归属权规则已实现（2026-09-23 业务确认：仓管只能修改自己录入的记录）。**
>
> 它此前确实是死代码：`editable_record()` 里写着
> `if current_operator_id() is not None and ...`，而 `current_operator_id()`
> **对所有角色恒返回 `None`**（为停用产品/供应商范围限制而做的有意决定）。
> **两个无关策略共用一个 helper，改一个把另一个也关掉了。**
>
> **修复方式与原先设想的相反**：不是让 `current_operator_id()` 恢复返回真实 id
> ——那会连带重启产品/供应商范围限制，影响面大得多且与本次决定无关。
> 而是让归属检查**直接读角色**（`current_user()["role"]`），只影响这一条策略。
>
> 归属数据本来就是好的：`completed_by_user_id` 写入时用的是
> `current_actor_id()`（返回真实 id），**只有校验那一环断了**。
>
> 无主记录（`completed_by_user_id` 为 NULL 的历史数据）仓管不可改，管理员可清理。
> 测试见 `tests/test_record_ownership.py`（9 项，已验证可证伪），
> 细节见 `SECURITY.md` §「记录归属权」。
...
| 第六 | 记录归属权规则未实现（仓管可互相校对） | ✅ 已实现（2026-09-23 业务确认：只能改自己的） | 本文档 §10 |
